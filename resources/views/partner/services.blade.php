...
@section('sidebar-nav')
    ...
    <a href="{{ route('partner.services') }}" class="nav-item active">
...
<div class="section-card mb-5">
    ...
    <form method="POST" action="{{ route('partner.services.store') }}" class="p-5">
        @csrf
        <div class="form-grid">
            <div class="form-group col-span-2">
                <label class="form-label">اسم الخدمة <span class="text-red-500">*</span></label>
                <input type="text" name="title" value="{{ old('title') }}"
                       class="form-input @error('title') border-red-500 @enderror" required placeholder="مثال: تصوير حفل زفاف">
                @error('title')<p class="form-error">{{ $message }}</p>@enderror
            </div>
            <div class="form-group">
                <label class="form-label">السعر</label>
                <input type="text" name="price" value="{{ old('price') }}" class="form-input" placeholder="مثال: يبدأ من 500 ريال">
            </div>
            <div class="form-group col-span-2">
    ...
    <div class="p-5">
        ...
        <div class="flex flex-col gap-3">
            ...
            <div class="flex items-start gap-4 p-4 bg-[#f8fdf9] rounded-xl border border-[#e2ede7]">
                ...
                @endif
                <form method="POST" action="{{ route('partner.services.delete', $svc->id) }}"
