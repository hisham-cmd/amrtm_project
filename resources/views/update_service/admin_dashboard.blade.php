...
/* SIDEBAR */
.sb{width:var(--sb-w);flex-shrink:0;background:linear-gradient(180deg,var(--hf) 0%,#0D1560 100%);display:flex;flex-direction:column;height:100vh;overflow-y:auto;z-index:50;}
...
.sb-item{display:flex;align-items:center;gap:9px;padding:.62rem .85rem;border-radius:10px;cursor:pointer;transition:all .2s;color:rgba(255,255,255,.55);font-size:13px;font-weight:600;margin-bottom:2px;}
...
.sb-badge{margin-right:auto;padding:2px 6px;border-radius:8px;background:rgba(198,40,40,.4);color:#fff;font-size:10px;font-weight:700;}
body.en .sb-badge{margin-right:0;margin-left:auto;}
.sb-bottom{padding:.7rem .6rem;border-top:1px solid rgba(255,255,255,.08);}
.sb-profile{display:flex;align-items:center;gap:8px;padding:.7rem .85rem;border-radius:10px;background:rgba(255,255,255,.08);}
.sb-av{width:34px;height:34px;border-radius:50%;object-fit:cover;border:2px solid rgba(255,255,255,.3);flex-shrink:0;background:var(--pri);display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:800;color:#fff;}
...
.tb-srch{position:relative;}
.tb-srch input{height:36px;width:220px;padding:0 36px 0 13px;border-radius:9px;border:1.5px solid var(--b1);background:var(--sur2);color:var(--t1);font-family:inherit;font-size:13px;outline:none;transition:border-color .2s;}
body.en .tb-srch input{padding:0 13px 0 36px;}
.tb-srch input:focus{border-color:var(--pri);}
.tb-srch input::placeholder{color:var(--t3);}
.tb-srch-ico{position:absolute;right:10px;top:50%;transform:translateY(-50%);color:var(--t3);font-size:15px;pointer-events:none;}
body.en .tb-srch-ico{right:auto;left:10px;}
...
.notif-badge{position:absolute;top:-3px;right:-3px;width:14px;height:14px;border-radius:50%;background:var(--red);border:2px solid var(--sur);font-size:8px;color:#fff;display:flex;align-items:center;justify-content:center;font-weight:700;}
...
.btn-pri{display:inline-flex;align-items:center;gap:6px;padding:8px 16px;border-radius:9px;background:var(--pri);color:#fff;font-family:inherit;font-size:13px;font-weight:700;cursor:pointer;border:none;box-shadow:0 3px 10px var(--sh2);transition:all .2s;}
...
/* STAT CARDS */
.stat-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(185px,1fr));gap:.9rem;margin-bottom:1.5rem;}
...
.sc-tr{font-size:11px;font-weight:700;display:flex;align-items:center;gap:3px;margin-top:4px;}
.sc-tr.up{color:var(--green);}
.sc-tr.dn{color:var(--red);}

/* CHARTS */
.charts-row{display:grid;grid-template-columns:1.6fr 1fr;gap:1rem;margin-bottom:1.5rem;}
.chart-card{background:var(--sur);border-radius:14px;border:1px solid var(--b1);padding:1.3rem;box-shadow:0 2px 8px var(--sh);}
.ch-ttl{font-size:14px;font-weight:700;color:var(--t1);margin-bottom:3px;}
.ch-sub{font-size:11.5px;color:var(--t3);margin-bottom:1rem;}
/* Bar chart */
.bar-chart{display:flex;align-items:flex-end;gap:.4rem;height:100px;position:relative;padding-bottom:20px;}
.bar-item{flex:1;display:flex;flex-direction:column;align-items:center;position:relative;}
.bar{width:100%;border-radius:5px 5px 0 0;transition:height .6s ease;cursor:pointer;position:relative;}
.bar:hover{filter:brightness(1.1);}
.bar-v{position:absolute;top:-16px;left:50%;transform:translateX(-50%);font-size:9px;font-weight:700;color:var(--t2);}
...
.donut-n{font-size:18px;font-weight:900;color:var(--t1);}
.donut-l{font-size:9px;color:var(--t3);}
.donut-legend{display:flex;flex-direction:column;gap:.5rem;}
.dl-item{display:flex;align-items:center;gap:.5rem;font-size:11.5px;color:var(--t2);}
.dl-dot{width:10px;height:10px;border-radius:50%;flex-shrink:0;}

/* TOP SERVICES */
.top-card{background:var(--sur);border-radius:14px;border:1px solid var(--b1);padding:1.3rem;box-shadow:0 2px 8px var(--sh);margin-bottom:1.5rem;}
.ts-row{display:flex;align-items:center;gap:.85rem;padding:.65rem 0;border-bottom:1px solid var(--bc);}
.ts-row:last-child{border-bottom:none;}
.ts-rank{width:22px;height:22px;border-radius:6px;background:var(--pd);display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:800;color:var(--pri);flex-shrink:0;}
.ts-ico{width:34px;height:34px;border-radius:9px;display:flex;align-items:center;justify-content:center;flex-shrink:0;}
.ts-ico i{font-size:16px;}
.ts-nm{font-size:12.5px;font-weight:700;color:var(--t1);flex:1;}
.ts-cat{font-size:10.5px;color:var(--t3);}
.ts-bar-w{width:80px;height:6px;border-radius:3px;background:var(--pd);overflow:hidden;}
.ts-bar{height:100%;border-radius:3px;}
.ts-cnt{font-size:12px;font-weight:700;color:var(--pri);min-width:28px;text-align:center;}

/* REQUESTS */
.req-filters{display:flex;gap:.6rem;flex-wrap:wrap;margin-bottom:1.1rem;}
.rf-btn{padding:6px 13px;border-radius:8px;font-size:12.5px;font-weight:600;cursor:pointer;border:1.5px solid var(--b1);background:transparent;color:var(--t2);transition:all .2s;font-family:inherit;}
.rf-btn.on{background:var(--pri);color:#fff;border-color:var(--pri);}
.rf-btn:hover:not(.on){background:var(--pd);}
.req-card{background:var(--sur);border-radius:14px;border:1px solid var(--b1);margin-bottom:.85rem;overflow:hidden;box-shadow:0 2px 8px var(--sh);}
.req-hd{display:flex;align-items:center;gap:.85rem;padding:.9rem 1.2rem;cursor:pointer;}
...
.req-time{font-size:11px;color:var(--t3);flex-shrink:0;}
.req-st{padding:4px 10px;border-radius:20px;font-size:11.5px;font-weight:700;flex-shrink:0;}
...
.req-dg{display:grid;grid-template-columns:repeat(auto-fill,minmax(170px,1fr));gap:.65rem;margin-bottom:1rem;}
.rd{background:var(--sur2);border-radius:9px;padding:.65rem .85rem;}
.rd-l{font-size:10px;color:var(--t3);margin-bottom:2px;}
.rd-v{font-size:13px;font-weight:700;color:var(--t1);}
/* Status actions */
.st-actions{display:flex;gap:.6rem;flex-wrap:wrap;margin-bottom:.85rem;}
.sa{display:inline-flex;align-items:center;gap:5px;padding:7px 13px;border-radius:9px;font-family:inherit;font-size:12.5px;font-weight:700;cursor:pointer;border:1.5px solid transparent;transition:all .2s;}
.sa.proc{background:rgba(2,119,189,.1);color:var(--blue);border-color:rgba(2,119,189,.2);}
.sa.proc:hover{background:var(--blue);color:#fff;}
.sa.inprog{background:rgba(249,168,37,.1);color:var(--yellow);border-color:rgba(249,168,37,.2);}
.sa.inprog:hover{background:var(--yellow);color:#fff;}
.sa.done{background:rgba(27,94,32,.1);color:var(--green);border-color:rgba(27,94,32,.2);}
.sa.done:hover{background:var(--green);color:#fff;}
.sa.rej{background:rgba(198,40,40,.1);color:var(--red);border-color:rgba(198,40,40,.2);}
.sa.rej:hover{background:var(--red);color:#fff;}
/* Set time */
.time-row{display:flex;gap:.7rem;align-items:center;margin-bottom:.7rem;flex-wrap:wrap;}
.time-inp{height:38px;padding:0 12px;border-radius:9px;border:1.5px solid var(--b1);background:var(--sur);color:var(--t1);font-family:inherit;font-size:13px;outline:none;flex:1;min-width:160px;transition:border-color .2s;}
.time-inp:focus{border-color:var(--pri);}
.time-btn{height:38px;padding:0 14px;border-radius:9px;background:var(--pri);color:#fff;font-family:inherit;font-size:13px;font-weight:700;cursor:pointer;border:none;white-space:nowrap;}
/* Reject note */
.rej-area{display:none;margin-top:.7rem;}
.rej-area.show{display:block;}
.rej-area textarea{width:100%;height:76px;padding:10px 12px;border-radius:10px;border:1.5px solid rgba(198,40,40,.3);background:rgba(198,40,40,.05);color:var(--t1);font-family:inherit;font-size:13px;outline:none;resize:none;margin-bottom:.6rem;transition:border-color .2s;}
.rej-area textarea:focus{border-color:var(--red);}
.rej-send{display:inline-flex;align-items:center;gap:6px;padding:7px 16px;border-radius:9px;background:var(--red);color:#fff;font-family:inherit;font-size:13px;font-weight:700;cursor:pointer;border:none;}
.sa.note{background:rgba(106,27,154,.1);color:var(--purple);border-color:rgba(106,27,154,.2);}
.sa.note:hover{background:var(--purple);color:#fff;}
.sa.info{background:rgba(230,81,0,.1);color:var(--orange);border-color:rgba(230,81,0,.2);}
.sa.info:hover{background:var(--orange);color:#fff;}
.note-area,.info-area{display:none;margin-top:.7rem;}
.note-area.show,.info-area.show{display:block;}
.note-area textarea,.info-area textarea{width:100%;height:76px;padding:10px 12px;border-radius:10px;border:1.5px solid rgba(106,27,154,.3);background:rgba(106,27,154,.05);color:var(--t1);font-family:inherit;font-size:13px;outline:none;resize:none;margin-bottom:.6rem;transition:border-color .2s;}
.info-area textarea{border-color:rgba(230,81,0,.3);background:rgba(230,81,0,.05);}
.note-send{display:inline-flex;align-items:center;gap:6px;padding:7px 16px;border-radius:9px;background:var(--purple);color:#fff;font-family:inherit;font-size:13px;font-weight:700;cursor:pointer;border:none;}
.info-send{display:inline-flex;align-items:center;gap:6px;padding:7px 16px;border-radius:9px;background:var(--orange);color:#fff;font-family:inherit;font-size:13px;font-weight:700;cursor:pointer;border:none;}
...
.notif-list-p{overflow-y:auto;flex:1;}
...
.notif-empty-p{padding:2rem;text-align:center;color:var(--t3);font-size:13px;}
/* Log */
.req-log-ttl{font-size:12px;font-weight:700;color:var(--t2);margin-bottom:.5rem;margin-top:.8rem;display:flex;align-items:center;gap:5px;}
.log-row{display:flex;gap:.7rem;padding:.45rem 0;border-bottom:1px solid var(--bc);font-size:12px;}
.log-row:last-child{border-bottom:none;}
.log-dot{width:8px;height:8px;border-radius:50%;flex-shrink:0;margin-top:3px;}
.log-txt{color:var(--t2);flex:1;line-height:1.5;}
.log-time{font-size:10.5px;color:var(--t3);}

/* PRICING */
.price-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:.9rem;}
.price-card{background:var(--sur);border-radius:14px;border:1px solid var(--b1);padding:1.2rem;box-shadow:0 2px 8px var(--sh);}
.pc-head{display:flex;align-items:center;gap:.7rem;margin-bottom:.9rem;padding-bottom:.7rem;border-bottom:1px solid var(--bc);}
.pc-ico{width:38px;height:38px;border-radius:10px;display:flex;align-items:center;justify-content:center;flex-shrink:0;}
.pc-nm{font-size:13px;font-weight:700;color:var(--t1);flex:1;}
.pc-ent{font-size:11px;color:var(--t3);}
.price-row{display:flex;align-items:center;gap:.7rem;}
.price-inp{flex:1;height:40px;padding:0 12px;border-radius:9px;border:1.5px solid var(--b1);background:var(--sur2);color:var(--t1);font-family:inherit;font-size:13.5px;font-weight:700;outline:none;transition:border-color .2s;}
.price-inp:focus{border-color:var(--pri);}
.price-unit{font-size:12px;color:var(--t3);flex-shrink:0;}
.price-save{height:40px;padding:0 14px;border-radius:9px;background:var(--pri);color:#fff;font-family:inherit;font-size:13px;font-weight:700;cursor:pointer;border:none;transition:all .2s;}
.price-save:hover{background:var(--pri2);}

/* FINANCE TABLE */
.fin-table-wrap{background:var(--sur);border-radius:14px;border:1px solid var(--b1);overflow:hidden;box-shadow:0 2px 8px var(--sh);}
.fin-row{display:flex;align-items:center;gap:.9rem;padding:.8rem 1.2rem;border-bottom:1px solid var(--bc);}
.fin-row:last-child{border-bottom:none;}
.fin-row:first-child{background:var(--sur2);font-size:11.5px;font-weight:700;color:var(--t3);}
.fin-ref{font-size:12.5px;font-weight:700;color:var(--pri);min-width:100px;}
.fin-svc{font-size:12.5px;color:var(--t1);flex:1;}
.fin-client{font-size:12.5px;color:var(--t2);min-width:130px;}
.fin-amt{font-size:13.5px;font-weight:800;min-width:90px;}
.fin-amt.credit{color:var(--green);}
.fin-status{min-width:90px;}
.fin-date{font-size:11px;color:var(--t3);min-width:90px;}

/* CATALOG MANAGEMENT */
.cat-tabs{display:flex;gap:.5rem;margin-bottom:1.4rem;border-bottom:2px solid var(--b1);padding-bottom:0;}
.cat-tab{padding:.6rem 1.1rem;font-size:13px;font-weight:700;color:var(--t3);cursor:pointer;border-bottom:2px solid transparent;margin-bottom:-2px;transition:all .2s;}
.cat-tab.on{color:var(--pri);border-color:var(--pri);}
.cat-tab-panel{display:none;}
.cat-tab-panel.on{display:block;}
.cat-add-form{background:var(--sur);border-radius:14px;border:1px solid var(--b1);padding:1.3rem;margin-bottom:1.2rem;box-shadow:0 2px 8px var(--sh);}
.cat-form-ttl{font-size:14px;font-weight:800;color:var(--t1);margin-bottom:1rem;display:flex;align-items:center;gap:7px;}
.cat-form-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:.8rem;margin-bottom:.9rem;}
.cat-form-grid input,.cat-form-grid select{height:42px;padding:0 12px;border-radius:9px;border:1.5px solid var(--b1);background:var(--sur2);color:var(--t1);font-family:inherit;font-size:13px;outline:none;width:100%;transition:border-color .2s;}
.cat-form-grid input:focus,.cat-form-grid select:focus{border-color:var(--pri);}
.cat-form-grid label{font-size:11.5px;font-weight:700;color:var(--t2);display:block;margin-bottom:4px;}
.cat-list{background:var(--sur);border-radius:14px;border:1px solid var(--b1);overflow:hidden;box-shadow:0 2px 8px var(--sh);}
.cat-list-hd{display:grid;align-items:center;padding:.7rem 1.2rem;background:var(--sur2);border-bottom:1px solid var(--b1);font-size:11.5px;font-weight:700;color:var(--t3);}
.cat-row{display:grid;align-items:center;padding:.75rem 1.2rem;border-bottom:1px solid var(--bc);transition:background .15s;}
.cat-row:last-child{border-bottom:none;}
.cat-row:hover{background:var(--sur2);}
.cat-row .ico-prev{width:34px;height:34px;border-radius:9px;display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:17px;}
.cat-nm{font-size:13px;font-weight:700;color:var(--t1);}
.cat-sub{font-size:11px;color:var(--t3);}
.cat-actions{display:flex;gap:.5rem;justify-content:flex-end;}
.cat-act-btn{height:30px;padding:0 10px;border-radius:7px;font-family:inherit;font-size:12px;font-weight:700;cursor:pointer;border:1.5px solid transparent;transition:all .2s;}
.cat-act-btn.edit{background:rgba(26,35,126,.08);color:var(--pri);border-color:var(--b1);}
.cat-act-btn.edit:hover{background:var(--pri);color:#fff;}
.cat-act-btn.del{background:rgba(198,40,40,.08);color:var(--red);border-color:rgba(198,40,40,.2);}
.cat-act-btn.del:hover{background:var(--red);color:#fff;}
.cat-act-btn.tog{background:rgba(27,94,32,.08);color:var(--green);border-color:rgba(27,94,32,.2);}
.cat-act-btn.tog:hover{background:var(--green);color:#fff;}
.cat-act-btn.tog.off{background:rgba(249,168,37,.08);color:var(--yellow);border-color:rgba(249,168,37,.2);}
.cat-act-btn.tog.off:hover{background:var(--yellow);color:#fff;}
.badge-count{display:inline-flex;align-items:center;justify-content:center;padding:2px 8px;border-radius:20px;background:rgba(26,35,126,.1);color:var(--pri);font-size:11px;font-weight:700;}
.cat-empty{padding:2.5rem;text-align:center;color:var(--t3);font-size:13px;}
.cat-status-dot{width:8px;height:8px;border-radius:50%;flex-shrink:0;}
.cat-status-dot.active{background:var(--green);}
.cat-status-dot.inactive{background:var(--t4);}
/* USER CARDS */
.usr-card{background:var(--sur);border-radius:14px;border:1px solid var(--b1);padding:.95rem 1.2rem;margin-bottom:.6rem;box-shadow:0 2px 8px var(--sh);display:grid;grid-template-columns:42px 1fr auto auto;align-items:center;gap:.9rem;transition:transform .15s;}
.usr-card:hover{transform:translateY(-2px);}
.usr-av{width:42px;height:42px;border-radius:50%;background:linear-gradient(135deg,var(--pri),var(--pri3));display:flex;align-items:center;justify-content:center;font-size:16px;font-weight:900;color:#fff;flex-shrink:0;}
.usr-nm{font-size:13.5px;font-weight:800;color:var(--t1);margin-bottom:3px;}
.usr-meta{font-size:11.5px;color:var(--t3);}
.usr-bal{font-size:15px;font-weight:900;text-align:center;}
.usr-bal.pos{color:var(--green);}
.usr-bal.neg{color:var(--red);}
.usr-actions{display:flex;gap:.4rem;}
/* LOG ROWS */
.log-entry{background:var(--sur);border-radius:12px;border:1px solid var(--b1);padding:.85rem 1.1rem;margin-bottom:.5rem;display:grid;grid-template-columns:36px 1fr auto;align-items:start;gap:.8rem;}
...
.log-main-nm{font-size:13px;font-weight:700;color:var(--t1);}
.log-main-meta{font-size:11.5px;color:var(--t3);margin-top:3px;}
.log-note{font-size:11.5px;color:var(--t2);margin-top:4px;font-style:italic;}
.log-date{font-size:10.5px;color:var(--t3);white-space:nowrap;}
/* ANALYTICS TOP SVC */
.an-svc-row{display:flex;align-items:center;gap:.8rem;padding:.6rem 0;border-bottom:1px solid var(--bc);}
.an-svc-row:last-child{border-bottom:none;}
.an-svc-bar-w{flex:1;height:8px;border-radius:4px;background:var(--sur2);overflow:hidden;}
.an-svc-bar{height:100%;border-radius:4px;transition:width .6s ease;}
/* OFFICE CARDS */
.off-office-card{background:var(--sur);border-radius:14px;border:1px solid var(--b1);padding:1.1rem 1.3rem;margin-bottom:.7rem;box-shadow:0 2px 8px var(--sh);display:grid;grid-template-columns:auto 1fr auto;gap:1rem;align-items:center;transition:transform .15s;}
.off-office-card:hover{transform:translateY(-2px);}
.off-type-badge{padding:3px 10px;border-radius:20px;font-size:10.5px;font-weight:700;white-space:nowrap;}
.off-type-badge.law{background:rgba(26,35,126,.1);color:#1A237E;}
.off-type-badge.services{background:rgba(2,119,189,.1);color:#0277BD;}
.off-type-badge.customs{background:rgba(0,105,92,.1);color:#00695C;}
.off-verify-badge{padding:3px 10px;border-radius:20px;font-size:10.5px;font-weight:700;}
.off-verify-badge.verified{background:rgba(27,94,32,.1);color:var(--green);}
.off-verify-badge.pending{background:rgba(249,168,37,.1);color:#F57F17;}
.off-verify-badge.inactive{background:rgba(198,40,40,.1);color:var(--red);}
.off-card-ico{width:46px;height:46px;border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:22px;flex-shrink:0;}
.off-card-meta{display:flex;gap:.5rem;flex-wrap:wrap;align-items:center;margin-top:4px;}
.off-card-actions{display:flex;gap:.5rem;flex-shrink:0;flex-wrap:wrap;justify-content:flex-end;}
/* ADMIN / PERMISSIONS CARDS */
.admin-card{background:var(--sur);border-radius:14px;border:1px solid var(--b1);padding:1.2rem 1.4rem;margin-bottom:.8rem;box-shadow:0 2px 8px var(--sh);}
.admin-card-hd{display:flex;align-items:center;gap:.9rem;margin-bottom:1rem;}
.admin-card-av{width:42px;height:42px;border-radius:50%;background:linear-gradient(135deg,var(--pri),var(--pri2));display:flex;align-items:center;justify-content:center;font-size:17px;font-weight:900;color:#fff;flex-shrink:0;}
.admin-card-nm{font-size:14px;font-weight:800;color:var(--t1);}
.admin-card-em{font-size:11.5px;color:var(--t3);}
.perm-row{display:flex;align-items:center;justify-content:space-between;padding:.6rem 0;border-bottom:1px solid var(--bc);}
.perm-row:last-child{border-bottom:none;}
.perm-lbl{font-size:13px;font-weight:600;color:var(--t2);}
.perm-lbl small{display:block;font-size:10.5px;color:var(--t3);font-weight:400;}
...
.perm-slider{position:absolute;inset:0;background:#ccc;border-radius:11px;cursor:pointer;transition:.3s;}
.perm-slider::before{content:'';position:absolute;width:16px;height:16px;bottom:3px;right:3px;background:#fff;border-radius:50%;transition:.3s;}
body.en .perm-slider::before{right:auto;left:3px;}
.perm-toggle input:checked+.perm-slider{background:var(--pri);}
.perm-toggle input:checked+.perm-slider::before{transform:translateX(-18px);}
body.en .perm-toggle input:checked+.perm-slider::before{transform:translateX(18px);}
/* MODAL */
.modal-ov{position:fixed;inset:0;background:rgba(0,0,0,.45);z-index:300;display:none;align-items:center;justify-content:center;}
.modal-ov.show{display:flex;}
.modal-box{background:var(--sur);border-radius:20px;padding:1.8rem;width:100%;max-width:460px;box-shadow:0 16px 64px rgba(26,35,126,.25);}
.modal-ttl{font-size:16px;font-weight:800;color:var(--t1);margin-bottom:1.2rem;display:flex;align-items:center;gap:.6rem;}
.modal-fld{margin-bottom:.9rem;}
.modal-fld label{font-size:12px;font-weight:700;color:var(--t2);display:block;margin-bottom:4px;}
.modal-fld input{width:100%;height:42px;padding:0 13px;border-radius:10px;border:1.5px solid var(--b1);background:var(--sur2);color:var(--t1);font-family:inherit;font-size:13.5px;outline:none;}
.modal-fld input:focus{border-color:var(--pri);}
.modal-btns{display:flex;gap:.7rem;justify-content:flex-end;margin-top:1.2rem;}
.btn-sec{height:38px;padding:0 16px;border-radius:9px;background:transparent;border:1.5px solid var(--b1);color:var(--t2);font-family:inherit;font-size:13px;font-weight:700;cursor:pointer;}
/* REVENUE CHART */
.rev-chart-row{display:flex;align-items:flex-end;gap:.35rem;height:120px;margin-top:1rem;}
.rev-bar-wrap{flex:1;display:flex;flex-direction:column;align-items:center;gap:3px;}
.rev-bar{width:100%;border-radius:5px 5px 0 0;background:linear-gradient(180deg,#1A237E,#1565C0);min-height:4px;transition:height .5s ease;}
.rev-bar-lbl{font-size:9px;color:var(--t3);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:100%;text-align:center;}
.rev-bar-val{font-size:9px;font-weight:700;color:var(--t2);}
...
@media(max-width:900px){
  .sb{width:60px;}
  .sb-logo-nm,.sb-logo-sb,.sb-item span,.sb-badge,.sb-sec,.sb-un,.sb-role{display:none;}
...
@media(max-width:580px){
  ...
  .req-dg{grid-template-columns:1fr 1fr;}
  .price-grid{grid-template-columns:1fr;}
}
/* ── Color Picker ── */
.cp-row{display:flex;gap:8px;flex-wrap:wrap;margin-top:6px;}
.cp-swatch{width:34px;height:34px;border-radius:9px;cursor:pointer;border:2.5px solid transparent;transition:border-color .15s,transform .15s,box-shadow .15s;flex-shrink:0;}
.cp-swatch:hover{transform:scale(1.13);}
.cp-swatch.on{border-color:#0D1257;box-shadow:0 0 0 2px #fff,0 0 0 4px #0D1257;}
.cp-sel-label{font-size:11px;color:var(--t3);margin-top:5px;font-weight:700;min-height:15px;}
</style>
</head>
<body class="ar">
<div class="layout">
...
<aside class="sb">
  <div class="sb-logo" onclick="location.href=(AMRTM_ROUTES&&AMRTM_ROUTES.home)||'/amrtm'">
    ...
    <div><div class="sb-logo-nm" id="sb-nm">آمر تم</div><div class="sb-logo-sb" id="sb-sb">لوحة التحكم</div></div>
  </div>
  <nav class="sb-nav">
...
@if(auth('business')->user()->role === 'supervisor')
    ...
    <div class="sb-item" onclick="showPage('offices')">
        ...
        <span class="sb-badge" id="sb-off-pend"
              style="display:none;background:rgba(249,168,37,.5);">
            0
        </span>
    </div>
...
@endif
  ...
  <div class="sb-bottom">
    <div class="sb-profile">
      ...
      <div style="flex:1;min-width:0;"><div class="sb-un" id="sb-un">المدير</div><div class="sb-role" id="sb-role">مدير النظام</div></div>
...
<div class="main">
  <div class="topbar">
    ...
    <div class="tb-right">
      <div class="tb-srch"><i class="ti ti-search tb-srch-ico"></i><input type="text" id="srch-inp" placeholder="بحث في الطلبات..."/></div>
      ...
      <form id="nb-logout-form"
      method="POST"
      action="{{ route('amrtm.logout') }}"
      style="display:none;">
    @csrf
</form>

<button
    onclick="document.getElementById('nb-logout-form').submit()"
...
@if(auth('business')->user()->role === 'supervisor')
    ...
    <div class="page on" id="page-overview">
      <div class="pg-hd">
        <div><div class="pg-ttl" id="ov-ttl">نظرة عامة على المنصة</div><div class="pg-sub" id="ov-sub">آخر تحديث: منذ لحظات</div></div>
        <button class="btn-pri" onclick="showPage('requests')"><i class="ti ti-file-text"></i><span id="ov-view-req">عرض الطلبات</span></button>
      </div>
      <div class="stat-grid">
        ...
        <div class="sc"><div class="sc-ico" style="background:rgba(2,119,189,.1)"><i class="ti ti-settings" style="color:var(--blue)"></i></div><div><div class="sc-n" id="sc-proc">0</div><div class="sc-l" id="sc-proc-l">جاري المعالجة</div></div></div>
        ...
        <div class="sc"><div class="sc-ico" style="background:rgba(198,40,40,.1)"><i class="ti ti-x" style="color:var(--red)"></i></div><div><div class="sc-n" id="sc-rej">0</div><div class="sc-l" id="sc-rej-l">مرفوضة</div></div></div>
        <div class="sc"><div class="sc-ico" style="background:rgba(106,27,154,.1)"><i class="ti ti-users" style="color:var(--purple)"></i></div><div><div class="sc-n" id="sc-users">0</div><div class="sc-l" id="sc-users-l">المستخدمين</div></div></div>
      </div>
          ...
          <div class="donut-wrap">
            <div class="donut">
              <svg viewBox="0 0 36 36">
                ...
                <circle id="d-proc" cx="18" cy="18" r="15.9" fill="none" stroke="var(--blue)" stroke-width="3" stroke-dasharray="0 100" stroke-dashoffset="25" stroke-linecap="round"/>
                <circle id="d-pend" cx="18" cy="18" r="15.9" fill="none" stroke="var(--orange)" stroke-width="3" stroke-dasharray="0 100" stroke-dashoffset="0" stroke-linecap="round"/>
                <circle id="d-done" cx="18" cy="18" r="15.9" fill="none" stroke="var(--green)" stroke-width="3" stroke-dasharray="0 100" stroke-dashoffset="0" stroke-linecap="round"/>
                <circle id="d-rej" cx="18" cy="18" r="15.9" fill="none" stroke="var(--red)" stroke-width="3" stroke-dasharray="0 100" stroke-dashoffset="0" stroke-linecap="round"/>
              </svg>
              <div class="donut-center"><div class="donut-n" id="d-total">0</div><div class="donut-l" id="d-lbl">طلب</div></div>
            </div>
            <div class="donut-legend">
              <div class="dl-item"><div class="dl-dot" style="background:var(--blue)"></div><span id="dl1">جاري (0)</span></div>
              <div class="dl-item"><div class="dl-dot" style="background:var(--orange)"></div><span id="dl2">انتظار (0)</span></div>
              <div class="dl-item"><div class="dl-dot" style="background:var(--green)"></div><span id="dl3">مكتملة (0)</span></div>
              <div class="dl-item"><div class="dl-dot" style="background:var(--red)"></div><span id="dl4">مرفوضة (0)</span></div>
            </div>
          </div>
        </div>
      </div>
...
<div class="page on" id="page-overview">
    ...
    <div class="page" id="page-requests">
      <div class="pg-hd">
        <div><div class="pg-ttl" id="req-ttl">قائمة الطلبات</div><div class="pg-sub" id="req-sub">إجمالي 0 طلب</div></div>
      </div>
    ...
    <div class="page" id="page-pricing">
      <div class="pg-hd"><div><div class="pg-ttl" id="price-ttl">إدارة التسعير</div><div class="pg-sub" id="price-sub">تحكم في أسعار الخدمات</div></div></div>
    ...
    <div class="page" id="page-finance">
      <div class="pg-hd">
        <div><div class="pg-ttl" id="fin-ttl">الحركة المالية</div><div class="pg-sub" id="fin-sub">جميع المعاملات المالية</div></div>
      ...
      <div class="stat-grid" style="grid-template-columns:repeat(4,1fr);margin-bottom:1.3rem;">
        <div class="sc"><div class="sc-ico" style="background:rgba(27,94,32,.1)"><i class="ti ti-trending-up" style="color:var(--green)"></i></div><div><div class="sc-n" id="fin-total">0</div><div class="sc-l" id="fin-total-l">إجمالي الإيرادات (ر.س)</div></div></div>
        <div class="sc"><div class="sc-ico" style="background:rgba(2,119,189,.1)"><i class="ti ti-calendar" style="color:var(--blue)"></i></div><div><div class="sc-n" id="fin-week">0</div><div class="sc-l" id="fin-week-l">هذا الأسبوع (ر.س)</div></div></div>
        <div class="sc"><div class="sc-ico" style="background:rgba(230,81,0,.1)"><i class="ti ti-receipt" style="color:var(--orange)"></i></div><div><div class="sc-n" id="fin-avg">0</div><div class="sc-l" id="fin-avg-l">متوسط قيمة الطلب</div></div></div>
        <div class="sc"><div class="sc-ico" style="background:rgba(249,168,37,.1)"><i class="ti ti-clock" style="color:var(--yellow)"></i></div><div><div class="sc-n" id="fin-pend">0</div><div class="sc-l" id="fin-pend-l">معلقة (ر.س)</div></div></div>
      </div>
    ...
    <div class="page" id="page-off-finance">
      <div class="pg-hd">
        <div><div class="pg-ttl">مالية المكاتب</div><div class="pg-sub">إيرادات عمولات المنصة من الطلبات المباشرة للمكاتب</div></div>
        <div style="display:flex;gap:8px;align-items:center">
          <input type="date" id="ofl-from" style="padding:7px 10px;border:1.5px solid #E0E0E0;border-radius:8px;font-size:12px;font-family:inherit">
          <input type="date" id="ofl-to"   style="padding:7px 10px;border:1.5px solid #E0E0E0;border-radius:8px;font-size:12px;font-family:inherit">
          <button class="btn-pri" onclick="loadOfficeFinancial()"><i class="ti ti-refresh"></i></button>
          <button class="btn-pri" style="background:#1B5E20" onclick="exportOfficeFinancialCSV()"><i class="ti ti-download"></i> تصدير</button>
        </div>
      </div>
      <div class="stat-grid" style="margin-bottom:1.3rem">
        <div class="sc"><div class="sc-ico" style="background:rgba(26,35,126,.1)"><i class="ti ti-files" style="color:var(--pri)"></i></div><div><div class="sc-n" id="ofl-total">—</div><div class="sc-l">إجمالي الطلبات</div></div></div>
        <div class="sc"><div class="sc-ico" style="background:rgba(27,94,32,.1)"><i class="ti ti-trending-up" style="color:var(--green)"></i></div><div><div class="sc-n" id="ofl-gross">—</div><div class="sc-l">إجمالي القيمة</div></div></div>
        <div class="sc"><div class="sc-ico" style="background:rgba(2,119,189,.1)"><i class="ti ti-coins" style="color:var(--blue)"></i></div><div><div class="sc-n" id="ofl-comm">—</div><div class="sc-l">عمولة المنصة</div></div></div>
        <div class="sc"><div class="sc-ico" style="background:rgba(230,81,0,.1)"><i class="ti ti-building-community" style="color:var(--orange)"></i></div><div><div class="sc-n" id="ofl-net">—</div><div class="sc-l">صافي للمكاتب</div></div></div>
        <div class="sc"><div class="sc-ico" style="background:rgba(27,94,32,.1)"><i class="ti ti-circle-check" style="color:var(--green)"></i></div><div><div class="sc-n" id="ofl-completed">—</div><div class="sc-l">طلبات مكتملة</div></div></div>
      </div>

      <!-- By Office Table -->
      <div style="background:var(--sur);border-radius:14px;border:1px solid var(--b1);box-shadow:0 2px 8px var(--sh);margin-bottom:1rem;overflow:hidden">
        <div style="padding:1rem 1.2rem;font-weight:800;font-size:14px;border-bottom:1px solid var(--b1)">أداء كل مكتب</div>
        <div style="overflow-x:auto"><table>
          <thead><tr><th>المكتب</th><th>العمولة %</th><th>الطلبات</th><th>الإجمالي</th><th>عمولة المنصة</th><th>الصافي للمكتب</th><th>مكتمل</th></tr></thead>
          <tbody id="ofl-by-office"><tr><td colspan="7" style="text-align:center;padding:2rem;color:var(--t3)">جارٍ التحميل...</td></tr></tbody>
        </table></div>
      </div>

      <!-- Monthly Chart -->
      <div style="background:var(--sur);border-radius:14px;border:1px solid var(--b1);box-shadow:0 2px 8px var(--sh);overflow:hidden">
        <div style="padding:1rem 1.2rem;font-weight:800;font-size:14px;border-bottom:1px solid var(--b1)">الأداء الشهري</div>
        <div style="overflow-x:auto"><table>
          <thead><tr><th>الشهر</th><th>الطلبات</th><th>إجمالي القيمة</th><th>عمولة المنصة</th></tr></thead>
          <tbody id="ofl-monthly"><tr><td colspan="4" style="text-align:center;padding:2rem;color:var(--t3)">جارٍ التحميل...</td></tr></tbody>
        </table></div>
      </div>
    </div>
    ...
    <div class="page" id="page-catalog">
      <div class="pg-hd">
        <div><div class="pg-ttl">اضافة الجهات</div><div class="pg-sub">أضف وعدّل التصنيفات والجهات والخدمات ديناميكياً</div></div>
      </div>
      ...
      <div class="cat-tab-panel on" id="cat-panel-categories">
        <div class="cat-add-form">
          ...
          <div class="cat-form-grid">
            <div><label>الاسم (عربي) *</label><input type="text" id="cat-name-ar" placeholder="مثال: الوزارات"/></div>
            <div><label>الاسم (إنجليزي) *</label><input type="text" id="cat-name-en" placeholder="e.g. Ministries"/></div>
            <div><label>المفتاح (key) *</label><input type="text" id="cat-key" placeholder="ministries"/></div>
            <div><label>الأيقونة *</label><div style="display:flex;gap:5px;"><input type="text" id="cat-icon" placeholder="ti-building-bank" style="flex:1;min-width:0;"/><button type="button" onclick="openIconPicker(document.getElementById('cat-icon'))" title="اختر من المكتبة" style="height:42px;padding:0 11px;border-radius:9px;border:1.5px solid rgba(26,35,126,.2);background:rgba(26,35,126,.06);color:#1A237E;cursor:pointer;flex-shrink:0;font-size:15px;line-height:1;"><i class="ti ti-photo"></i></button></div></div>
        ...
        <div class="cat-list">
          <div class="cat-list-hd" style="grid-template-columns:36px 1fr 1fr 80px 60px 120px;">
            <div></div><div>الاسم</div><div>المفتاح</div><div>الجهات</div><div>الحالة</div><div style="text-align:left;">إجراءات</div>
          </div>
          <div id="cat-list-body"><div class="cat-empty">جارٍ التحميل...</div></div>
        </div>
      </div>
        ...
        <div class="cat-add-form">
          ...
          <div class="cat-form-grid">
            <div><label>التصنيف *</label>
              <select id="ent-category-id"><option value="">-- اختر تصنيفاً --</option></select>
            </div>
            <div><label>الاسم (عربي) *</label><input type="text" id="ent-name-ar" placeholder="مثال: وزارة الداخلية"/></div>
            <div><label>الاسم (إنجليزي) *</label><input type="text" id="ent-name-en" placeholder="e.g. Ministry of Interior"/></div>
            <div><label>الأيقونة *</label><div style="display:flex;gap:5px;"><input type="text" id="ent-icon" placeholder="ti-shield" style="flex:1;min-width:0;"/><button type="button" onclick="openIconPicker(document.getElementById('ent-icon'))" title="اختر من المكتبة" style="height:42px;padding:0 11px;border-radius:9px;border:1.5px solid rgba(26,35,126,.2);background:rgba(26,35,126,.06);color:#1A237E;cursor:pointer;flex-shrink:0;font-size:15px;line-height:1;"><i class="ti ti-photo"></i></button></div></div>
...
<div class="form-group">
    ...
    <input
        ...
        id="ent-image"
        accept="image/*"
        onchange="previewEntityImage(this)"
    >

    <img
        id="ent-image-preview"
        src=""
        style="
            display:none;
            width:70px;
    ...
    >
            ...
            <div><label>التاق (عربي)</label><input type="text" id="ent-tag-ar" placeholder="الأمن والمواطنة"/></div>
            <div><label>التاق (إنجليزي)</label><input type="text" id="ent-tag-en" placeholder="Security &amp; Citizenship"/></div>
          ...
          <div style="display:flex;gap:.8rem;flex-wrap:wrap;align-items:center;margin-bottom:.8rem;">
            ...
            <select id="ent-filter-cat" style="height:38px;padding:0 12px;border-radius:9px;border:1.5px solid var(--b1);background:var(--sur2);color:var(--t1);font-family:inherit;font-size:13px;outline:none;" onchange="loadEntities(this.value)">
        ...
        <div class="cat-list">
          <div class="cat-list-hd" style="grid-template-columns:36px 1fr 1fr 80px 60px 120px;">
            <div></div><div>الاسم</div><div>التصنيف</div><div>الخدمات</div><div>الحالة</div><div style="text-align:left;">إجراءات</div>
          </div>
          <div id="ent-list-body"><div class="cat-empty">جارٍ التحميل...</div></div>
        </div>
      </div>
      ...
      <div class="cat-tab-panel" id="cat-panel-services">
        <div class="cat-add-form">
          ...
          <div class="cat-form-grid">
            <div><label>الجهة *</label>
              <select id="svc-entity-id"><option value="">-- اختر جهة --</option></select>
            </div>
            <div><label>الاسم (عربي) *</label><input type="text" id="svc-name-ar" placeholder="مثال: استخراج جواز السفر"/></div>
            <div><label>الاسم (إنجليزي) *</label><input type="text" id="svc-name-en" placeholder="e.g. Passport Issuance"/></div>
            <div><label>الأيقونة *</label><div style="display:flex;gap:5px;"><input type="text" id="svc-icon" placeholder="ti-id" style="flex:1;min-width:0;"/><button type="button" onclick="openIconPicker(document.getElementById('svc-icon'))" title="اختر من المكتبة" style="height:42px;padding:0 11px;border-radius:9px;border:1.5px solid rgba(26,35,126,.2);background:rgba(26,35,126,.06);color:#1A237E;cursor:pointer;flex-shrink:0;font-size:15px;line-height:1;"><i class="ti ti-photo"></i></button></div></div>
            <div><label>السعر (ر.س) *</label><input type="number" id="svc-price" placeholder="300" min="0" step="0.01"/></div>
            <div><label>أيام الإنجاز المتوقعة *</label><input type="number" id="svc-days" placeholder="5" min="1"/></div>
            <div><label>وصف (عربي)</label><input type="text" id="svc-desc-ar" placeholder="وصف الخدمة..."/></div>
            <div><label>وصف (إنجليزي)</label><input type="text" id="svc-desc-en" placeholder="Service description..."/></div>
          ...
          <div style="display:flex;gap:.8rem;flex-wrap:wrap;align-items:center;margin-bottom:.8rem;">
            ...
            <select id="svc-filter-ent" style="height:38px;padding:0 12px;border-radius:9px;border:1.5px solid var(--b1);background:var(--sur2);color:var(--t1);font-family:inherit;font-size:13px;outline:none;" onchange="loadSvcList(this.value)">
              <option value="">الكل</option>
            </select>
          </div>
        ...
        <div class="cat-list">
          <div class="cat-list-hd" style="grid-template-columns:36px 1fr 1fr 80px 70px 60px 120px;">
            <div></div><div>الخدمة</div><div>الجهة</div><div>السعر</div><div>الأيام</div><div>الحالة</div><div style="text-align:left;">إجراءات</div>
          </div>
          <div id="svc-list-body"><div class="cat-empty">جارٍ التحميل...</div></div>
        </div>
      </div>
    ...
    <div class="page" id="page-users">
      <div class="pg-hd">
        <div><div class="pg-ttl" id="usr-pg-ttl">إدارة المستخدمين</div><div class="pg-sub" id="usr-pg-sub">جميع المستخدمين المسجلين في المنصة</div></div>
        <button class="btn-pri" onclick="exportUsersCSV()"><i class="ti ti-download"></i><span>تصدير CSV</span></button>
      </div>
      <div class="stat-grid" style="margin-bottom:1.3rem;">
        <div class="sc"><div class="sc-ico" style="background:rgba(26,35,126,.1)"><i class="ti ti-users" style="color:var(--pri)"></i></div><div><div class="sc-n" id="usr-sc-total">0</div><div class="sc-l">إجمالي المستخدمين</div></div></div>
        <div class="sc"><div class="sc-ico" style="background:rgba(27,94,32,.1)"><i class="ti ti-user-check" style="color:var(--green)"></i></div><div><div class="sc-n" id="usr-sc-active">0</div><div class="sc-l">مستخدمون نشطون</div></div></div>
        <div class="sc"><div class="sc-ico" style="background:rgba(198,40,40,.1)"><i class="ti ti-user-off" style="color:var(--red)"></i></div><div><div class="sc-n" id="usr-sc-banned">0</div><div class="sc-l">محظورون</div></div></div>
        <div class="sc"><div class="sc-ico" style="background:rgba(2,119,189,.1)"><i class="ti ti-user-plus" style="color:var(--blue)"></i></div><div><div class="sc-n" id="usr-sc-new">0</div><div class="sc-l">جدد هذا الشهر</div></div></div>
      </div>
      ...
      <div style="display:flex;gap:.7rem;margin-bottom:1rem;">
        <div class="tb-srch" style="flex:1;max-width:340px;"><i class="ti ti-search tb-srch-ico"></i><input type="text" id="usr-srch" placeholder="ابحث بالاسم أو البريد أو الجوال..." oninput="debounceUserSearch(this.value)"/></div>
      </div>
      <div id="usr-list"><div style="text-align:center;padding:3rem;color:var(--t3);">جارٍ التحميل...</div></div>
    </div>

    <!-- ANALYTICS -->
    <div class="page" id="page-analytics">
      <div class="pg-hd">
        <div><div class="pg-ttl" id="an-pg-ttl">التحليلات والتقارير</div><div class="pg-sub">إحصائيات المنصة والأداء</div></div>
        <button class="btn-pri" onclick="exportAnalyticsCSV()"><i class="ti ti-download"></i><span>تصدير</span></button>
      </div>
      <div class="stat-grid" style="margin-bottom:1.3rem;">
        <div class="sc"><div class="sc-ico" style="background:rgba(27,94,32,.1)"><i class="ti ti-trending-up" style="color:var(--green)"></i></div><div><div class="sc-n" id="an-revenue">0</div><div class="sc-l">إجمالي الإيرادات (ر.س)</div></div></div>
        <div class="sc"><div class="sc-ico" style="background:rgba(26,35,126,.1)"><i class="ti ti-circle-check" style="color:var(--pri)"></i></div><div><div class="sc-n" id="an-rate">0%</div><div class="sc-l">معدل إتمام الطلبات</div></div></div>
        <div class="sc"><div class="sc-ico" style="background:rgba(198,40,40,.1)"><i class="ti ti-x" style="color:var(--red)"></i></div><div><div class="sc-n" id="an-rej-rate">0%</div><div class="sc-l">معدل الرفض</div></div></div>
      </div>
    ...
    <div class="page" id="page-logs">
      <div class="pg-hd">
        <div><div class="pg-ttl">سجل النشاط</div><div class="pg-sub">جميع الإجراءات المتخذة على الطلبات</div></div>
      </div>
      <div class="req-filters" style="margin-bottom:1rem;">
      ...
      <div style="display:flex;gap:.7rem;margin-bottom:1rem;">
        <div class="tb-srch" style="flex:1;max-width:340px;"><i class="ti ti-search tb-srch-ico"></i><input type="text" id="log-srch" placeholder="ابحث برقم الطلب أو اسم العميل..." oninput="debounceLogSearch(this.value)"/></div>
      </div>
      <div id="log-list"><div style="text-align:center;padding:3rem;color:var(--t3);">جارٍ التحميل...</div></div>
    </div>

    <!-- OFFICES -->
    <div class="page" id="page-offices">
      <div class="pg-hd">
        <div><div class="pg-ttl" id="off-pg-ttl">إدارة المكاتب</div><div class="pg-sub" id="off-pg-sub">مكاتب القطاع الخاص المسجلة في المنصة</div></div>
      </div>
      <div class="stat-grid" style="margin-bottom:1.3rem;" id="off-stats-grid">
        <div class="sc"><div class="sc-ico" style="background:rgba(2,119,189,.1)"><i class="ti ti-building" style="color:var(--blue)"></i></div><div><div class="sc-n" id="off-sc-total">0</div><div class="sc-l">إجمالي المكاتب</div></div></div>
        <div class="sc"><div class="sc-ico" style="background:rgba(27,94,32,.1)"><i class="ti ti-circle-check" style="color:var(--green)"></i></div><div><div class="sc-n" id="off-sc-verified">0</div><div class="sc-l">معتمدة ونشطة</div></div></div>
        <div class="sc"><div class="sc-ico" style="background:rgba(249,168,37,.1)"><i class="ti ti-clock" style="color:var(--yellow)"></i></div><div><div class="sc-n" id="off-sc-pending">0</div><div class="sc-l">تنتظر الاعتماد</div></div></div>
        <div class="sc"><div class="sc-ico" style="background:rgba(198,40,40,.1)"><i class="ti ti-ban" style="color:var(--red)"></i></div><div><div class="sc-n" id="off-sc-inactive">0</div><div class="sc-l">موقوفة</div></div></div>
      </div>
      <div class="req-filters" style="margin-bottom:1rem;">
      ...
      <div class="off-type-tabs" style="display:flex;gap:.5rem;flex-wrap:wrap;margin-bottom:1rem;">
        <button class="rf-btn on" style="font-size:11px;" onclick="filterOfficeType('all',this)">جميع الأنواع</button>
        <button class="rf-btn" style="font-size:11px;" onclick="filterOfficeType('law',this)">محاماة</button>
        <button class="rf-btn" style="font-size:11px;" onclick="filterOfficeType('services',this)">تعقيب وخدمات</button>
        <button class="rf-btn" style="font-size:11px;" onclick="filterOfficeType('customs',this)">جمارك</button>
        <button class="rf-btn" style="font-size:11px;" onclick="filterOfficeType('accounting',this)">محاسبين</button>
        <button class="rf-btn" style="font-size:11px;" onclick="filterOfficeType('engineering',this)">هندسة</button>
        <button class="rf-btn" style="font-size:11px;" onclick="filterOfficeType('freelance',this)">اصحاب مهن</button>
      </div>
      <div id="off-list"><div style="text-align:center;padding:3rem;color:var(--t3);">جارٍ التحميل...</div></div>
    </div>
...
<div class="page" id="page-office-specialties">
    ...
    ">
        ...
        <select
            id="specialty-filter-type"
            onchange="loadOfficeSpecialties(this.value)"
            style="
                height:38px;
                padding:0 12px;
                border-radius:9px;
    ...
    <div class="page" id="page-permissions">
      <div class="pg-hd">
        <div><div class="pg-ttl" id="perms-pg-ttl">إدارة الصلاحيات</div><div class="pg-sub" id="perms-pg-sub">منح وسحب صلاحيات المدراء</div></div>
        <button class="btn-pri" onclick="showCreateAdminModal()"><i class="ti ti-user-plus"></i><span>إضافة مدير</span></button>
      </div>
      <div id="admins-list"><div style="text-align:center;padding:3rem;color:var(--t3);">جارٍ التحميل...</div></div>
    </div>

    <!-- SETTINGS -->
    <div class="page" id="page-settings">
      <div class="pg-hd"><div><div class="pg-ttl" id="set-ttl">الإعدادات</div></div></div>
      <div class="chart-card" style="max-width:520px;">
        ...
        <div style="display:flex;flex-direction:column;gap:1rem;">
          <div><label style="font-size:12.5px;font-weight:700;color:var(--t1);display:block;margin-bottom:5px;" id="set-lbl-nm">الاسم</label><input type="text" value="مدير النظام" style="width:100%;height:44px;padding:0 13px;border-radius:10px;border:1.5px solid var(--b1);background:var(--sur2);color:var(--t1);font-family:inherit;font-size:13.5px;outline:none;"/></div>
          <div><label style="font-size:12.5px;font-weight:700;color:var(--t1);display:block;margin-bottom:5px;" id="set-lbl-em">البريد الإلكتروني</label><input type="email" value="admin@amrtm.com.sa" style="width:100%;height:44px;padding:0 13px;border-radius:10px;border:1.5px solid var(--b1);background:var(--sur2);color:var(--t1);font-family:inherit;font-size:13.5px;outline:none;"/></div>
          <button class="btn-pri" style="align-self:flex-start;" id="set-save"><i class="ti ti-check"></i><span id="set-save-l">حفظ التغييرات</span></button>
        </div>
      </div>
    </div>
...
<div class="modal-ov" id="balance-modal">
  <div class="modal-box">
    <div class="modal-ttl"><i class="ti ti-wallet" style="color:var(--pri)"></i><span id="bal-modal-ttl">تعديل الرصيد</span></div>
    <div style="background:var(--sur2);border-radius:10px;padding:.8rem 1rem;margin-bottom:1rem;display:flex;justify-content:space-between;align-items:center;">
      <span style="font-size:13px;font-weight:700;color:var(--t2);" id="bal-user-name">—</span>
      <span style="font-size:13px;color:var(--t3);">الرصيد الحالي: <strong id="bal-current" style="color:var(--pri)">0 ر.س</strong></span>
    </div>
    <div class="modal-fld">
      <label>نوع العملية</label>
      <div style="display:flex;gap:.5rem;">
        <label style="flex:1;display:flex;align-items:center;gap:.5rem;padding:.7rem;border-radius:9px;border:1.5px solid var(--b1);cursor:pointer;" id="bal-charge-lbl">
          <input type="radio" name="bal-type" value="charge" checked onchange="updateBalTypeUI()"/> <span style="font-size:13px;font-weight:700;color:var(--green);">شحن رصيد</span>
        </label>
        <label style="flex:1;display:flex;align-items:center;gap:.5rem;padding:.7rem;border-radius:9px;border:1.5px solid var(--b1);cursor:pointer;" id="bal-deduct-lbl">
          <input type="radio" name="bal-type" value="payment" onchange="updateBalTypeUI()"/> <span style="font-size:13px;font-weight:700;color:var(--red);">خصم رصيد</span>
        </label>
      </div>
    </div>
    <div class="modal-fld"><label>المبلغ (ر.س)</label><input type="number" id="bal-amount" min="0.01" step="0.01" placeholder="0.00"/></div>
    <div class="modal-fld"><label>السبب (اختياري)</label><input type="text" id="bal-reason" placeholder="سبب التعديل..."/></div>
    <div class="modal-btns">
      <button class="btn-sec" onclick="closeBalanceModal()">إلغاء</button>
      <button class="btn-pri" id="bal-submit-btn" onclick="doAdjustBalance()"><i class="ti ti-check"></i>تأكيد</button>
    </div>
  </div>
</div>
...
<div class="modal-ov" id="create-admin-modal">
  <div class="modal-box">
    ...
    <div class="modal-fld"><label>الاسم</label><input type="text" id="new-admin-name" placeholder="الاسم الكامل"/></div>
    <div class="modal-fld"><label>البريد الإلكتروني</label><input type="email" id="new-admin-email" placeholder="admin@example.com"/></div>
    <div class="modal-fld"><label>رقم الجوال</label><input type="tel" id="new-admin-phone" placeholder="05xxxxxxxx"/></div>
    <div class="modal-fld"><label>كلمة المرور</label><input type="password" id="new-admin-pass" placeholder="8 أحرف على الأقل"/></div>
    <div class="modal-btns">
